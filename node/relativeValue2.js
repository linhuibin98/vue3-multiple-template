...
// }
async function main() {
    const {workbook, worksheet} = await readXlsx('c2.xlsx', 'Sheet1');
    // 初步整理数据
    const result = {};
    worksheet.getSheetValues().forEach((row, index) => {
        if (row && index > 1) {
            let [ _, plot, name, a, b, abundance ] = row;
            b = parseFloat(String(b).trim());
            a = parseFloat(String(a).trim());
            if (!result[plot]) {
                result[plot] = {
                    allTotal: 0,
                    allBasalArea: 0
            ...
            // 截面
            const rv = Math.pow((a+b)/2, 2) * Math.PI * parseInt(abundance);
